...
A small Windows tray app with two features:

- A click-through topmost ring that follows the cursor, with color-coded click ripples (left / right / middle).
- A global screen dimmer that uses the Magnification API's `MagSetFullscreenColorEffect` to scale RGB output before scan-out, so it dims basically everything (including popups, context menus, and top-most windows) except the hardware cursor.
